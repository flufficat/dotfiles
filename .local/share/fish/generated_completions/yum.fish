...
complete -c yum -s C -l cacheonly -d 'Run entirely from system cache, don\\(aqt update the cache and use it even in …'
...
complete -c yum -l nodocs -d 'Do not install documentation.  Sets the rpm flag \\(aqRPMTRANS_FLAG_NODOCS\\(aq'
...
complete -c yum -l obsoletes -d 'This option has an effect on an install/update, it enables dnf\\(aqs obsoletes…'
